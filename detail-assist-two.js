    ...
    connection.query(sqlFDName, async(err, result, fields) => {
        if (err) {
            ...
            if (result.length > 0) {
                ...
                var sqlFNC = "SELECT `code`, link FROM ffp_detail WHERE dir_name = '" + currentDir + "' AND (content IS NULL OR content='') ORDER BY `code` DESC LIMIT 3";
                // console.log(sqlFNC);
